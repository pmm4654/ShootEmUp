...
public class MapGenerator : MonoBehaviour
{
    ...
    public void GenerateMap()
    {
        ...
        // creating nav mesh mask to keep enemies from going off the map
        ...
        navMeshFloor.localScale = new Vector3(maxMapSize.x, maxMapSize.y) * tileSize; // since the quad is rotated 90 degrees, the normal z axis is basically the y axis
    ...
    bool MapIsFullyAccessible(bool[,] obstacleMap, int currentObstacleCount )
    {
        ...
        while (queue.Count > 0)
        {
            Coord tile = queue.Dequeue(); // get a tile out of the queue
            for(int x = -1; x <= 1; x ++) // check its x neighbors
            {
                for(int y = -1; y <= 1; y++) // check its y neighbors
                {
                    int neighborX = tile.x + x;
                    int neighborY = tile.y + y;
                    if(x == 0 || y == 0) // make sure it's not the diagonals
                    {
        ...
        // you want to be able to go to as many tiles are there are available
        // so we check how many spaces there are without obstacles and we make sure that number is the same number that is available
